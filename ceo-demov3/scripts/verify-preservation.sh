...
ceo_demo_hash="$(
  ...
  find . -type f \
    ...
    ! -path './output/*' \
    ! -path './.playwright-cli/*' \
    -print | LC_ALL=C sort | while IFS= read -r file; do shasum -a 256 "$file"; done | shasum -a 256 | awk '{print $1}'
)"
if [[ "$ceo_demo_hash" != "5504b1ed48448be6334ef7a232d217f1dd11f8966b99a41f40b85c5f85e3aa7c" ]]; then
  echo "FAIL  ceo-demo/**  expected=5504b1ed48448be6334ef7a232d217f1dd11f8966b99a41f40b85c5f85e3aa7c actual=$ceo_demo_hash"
  exit 1
fi
echo "PASS  ceo-demo/**"
